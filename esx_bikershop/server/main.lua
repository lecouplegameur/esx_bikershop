...
TriggerEvent('esx_phone:registerNumber', 'bikerdeal', _U('dealer_customers'), false, false)
TriggerEvent('esx_society:registerSociety', 'bikerdeal', _U('car_dealer'), 'society_bikerdeal', 'society_bikerdeal', 'society_bikerdeal', {type = 'private'})
...
MySQL.ready(function()
	Categories     = MySQL.Sync.fetchAll('SELECT * FROM bikerdeal_categories')
	local vehicles = MySQL.Sync.fetchAll('SELECT * FROM bikerdeal')
...
AddEventHandler('esx_bikershop:setVehicleOwnedPlayerId', function (playerId, vehicleProps)
	...
	MySQL.Async.execute('INSERT INTO owned_bikerdeal (owner, plate, vehicle) VALUES (@owner, @plate, @vehicle)',
	{
		['@owner']   = xPlayer.identifier,
		['@plate']   = vehicleProps.plate,
		['@vehicle'] = json.encode(vehicleProps)
...
AddEventHandler('esx_bikershop:sellVehicle', function (vehicle)
	MySQL.Async.fetchAll('SELECT * FROM bikerdeal_vehicles WHERE vehicle = @vehicle LIMIT 1', {
		['@vehicle'] = vehicle
	}, function (result)
		local id = result[1].id

		MySQL.Async.execute('DELETE FROM bikerdeal_vehicles WHERE id = @id', {
			['@id'] = id
		})
	end)
end)
...
AddEventHandler('esx_bikershop:addToList', function(target, model, plate)
	...
	if xPlayer.job.name ~= 'bikerdeal' then
...
AddEventHandler('esx_bikershop:rentVehicle', function (vehicle, plate, playerName, basePrice, rentPrice, target)
	...
	MySQL.Async.fetchAll('SELECT * FROM bikerdeal_vehicles WHERE vehicle = @vehicle LIMIT 1', {
		...
		MySQL.Async.execute('INSERT INTO rented_bikerdeal (vehicle, plate, player_name, base_price, rent_price, owner) VALUES (@vehicle, @plate, @player_name, @base_price, @rent_price, @owner)',
		{
			['@vehicle']     = vehicle,
			['@plate']       = plate,
...
AddEventHandler('esx_bikershop:getStockItem', function (itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_bikerdeal', function (inventory)
...
ESX.RegisterServerCallback('esx_bikershop:buyVehicleSociety', function (source, cb, society, vehicleModel)
	...
	TriggerEvent('esx_addonaccount:getSharedAccount', 'society_' .. society, function (account)
		if account.money >= vehicleData.price then
			...
			MySQL.Async.execute('INSERT INTO bikerdeal_vehicles (vehicle, price) VALUES (@vehicle, @price)', {
...
ESX.RegisterServerCallback('esx_bikershop:getCommercialVehicles', function (source, cb)
	MySQL.Async.fetchAll('SELECT * FROM bikerdeal_vehicles ORDER BY vehicle ASC', {}, function (result)
		local vehicles = {}
...
AddEventHandler('esx_bikershop:returnProvider', function(vehicleModel)
	...
	MySQL.Async.fetchAll('SELECT * FROM bikerdeal_vehicles WHERE vehicle = @vehicle LIMIT 1', {
		...
		if result[1] then
			...
			TriggerEvent('esx_addonaccount:getSharedAccount', 'society_bikerdeal', function(account)
...
ESX.RegisterServerCallback('esx_bikershop:getRentedVehicles', function (source, cb)
	MySQL.Async.fetchAll('SELECT * FROM rented_bikerdeal ORDER BY player_name ASC', {}, function (result)
		local vehicles = {}
...
ESX.RegisterServerCallback('esx_bikershop:giveBackVehicle', function (source, cb, plate)
	MySQL.Async.fetchAll('SELECT * FROM rented_bikerdeal WHERE plate = @plate', {
		['@plate'] = plate
	}, function (result)
		if result[1] ~= nil then
			...
			MySQL.Async.execute('DELETE FROM rented_bikerdeal WHERE plate = @plate', {
				['@plate'] = plate
			})
...
AddEventHandler('esx_bikershop:setJobVehicleState', function(plate, state)
	...
	MySQL.Async.execute('UPDATE owned_bikerdeal SET `stored` = @stored WHERE plate = @plate AND job = @job', {
		['@stored'] = state,
		['@plate'] = plate,
		['@job'] = xPlayer.job.name
	}, function(rowsChanged)
...
function PayRent(d, h, m)
	MySQL.Async.fetchAll('SELECT * FROM rented_bikerdeal', {}, function (result)
		for i=1, #result, 1 do
...
RegisterServerEvent('AnnounceConcessMotoOuvert')
AddEventHandler('AnnounceConcessMotoOuvert', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local xPlayers	= ESX.GetPlayers()
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Concessionnaire Moto', '~g~Ouverture', 'Le concessionnaire Moto vient d\'ouvrir! Venez acheter vos véhicules!', 'CHAR_BIKESITE', 8)
	end
end)

RegisterServerEvent('AnnounceConcessMotoFerme')
AddEventHandler('AnnounceConcessMotoFerme', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local xPlayers	= ESX.GetPlayers()
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Concessionnaire Moto', '~r~Fermeture', 'Le concessionnaire Moto vient de fermer, revenez demain .', 'CHAR_BIKESITE', 8)
	end
end)
...
RegisterCommand('transfervehicle', function(source, args)
	...
	MySQL.Async.fetchAll(
        'SELECT * FROM owned_bikerdeal WHERE plate = @plate',
        {
            ['@plate'] = plate
        },
        function(result)
            if result[1] ~= nil then
				...
				if myID == CarOwner then
					...
					data = {}
						...
						MySQL.Sync.execute("UPDATE owned_bikerdeal SET owner=@owner WHERE plate=@plate", {['@owner'] = targetSteam, ['@plate'] = plate})
